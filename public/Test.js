...
// windowWidth;
food.style.left =
    Math.floor(Math.random() * Math.floor(100 / parseInt(food.style.width))) *
        parseInt(food.style.width) +
    'vw';
food.style.top = Math.floor(
                     Math.random() *
                     Math.floor(windowHeight / parseInt(food.style.height))) *
        parseInt(food.style.height) +
    'vw';
// document.getElementById("Sahil_info").innerHTML = food.style.left + " " +
// food.style.top + " " + food.style.width + " " + food.style.height;
...
function new_food_coord() {
  first: while (true) {
    food.style.left =
        Math.floor(
            Math.random() * Math.floor(100 / parseInt(food.style.width))) *
            parseInt(food.style.width) +
        'vw';
    food.style.top =
        Math.floor(
    ...
    for (i = 0; i < elem.length; i++)
      if (elem[i].style.left == food.style.left &&
          elem[i].style.top == food.style.top)
        continue first;
    break;
  }
}
function check_food() {
  if (food.style.left == elem[0].style.left &&
      food.style.top == elem[0].style.top) {
    new_food_coord();
    if (delay > 30) delay -= 2;
...
function check_collision() {
  for (i = 1; i < elem.length; i++)
    if (elem[0].style.left == elem[i].style.left &&
        elem[0].style.top == elem[i].style.top)
...
setInterval(function() {
  ...
  // document.getElementById("Sahil_info").innerHTML = "Hye " + elem.length;
  // document.getElementById("Sahil_info").innerHTML = food.style.left + " " +
  // food.style.top + " " + Math.random();
  document.onkeydown = function(event) {
    for (var i = 0; i < 4; i++) {
      if (i == event.keyCode - 37 && ((lastKeyCode - 37) % 2) != (i % 2) &&
          nextInterval == true) {
        nextInterval = false;
        // document.getElementById("Sahil_info").innerHTML = ((lastKeyCode - 37)
        // % 2) + " " + i;
        lastKeyCode = event.keyCode;
        temp_left = (1 - (i % 2)) * (i + -1);
        temp_top = (i % 2) * (i - 2);
        if (elem.length < 4 && elem_to_add < 4) elem_to_add += 4;
      }
    }
  };
  ...
  if (left < 0) left = 100 - width - 1;
  top = (top + temp_top) % windowHeight;
  if (top < 0) top = windowHeight - height;
  check_food();
  check_collision();
  insert_elem();
  process_elem();
  /*   document.getelem[0]entById("Sahil_info").innerHTML = temp_left + " " +
   * temp_top + " " + left + " " + top; */
