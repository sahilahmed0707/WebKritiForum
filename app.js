 ...
 con.connect(function (err) {
   ...
   var sql =
     'CREATE TABLE IF NOT EXISTS `users` ( `name` varchar(50) NOT NULL, `email` varchar(50) NOT NULL, `username` varchar(30) NOT NULL, `password` blob NOT NULL, `question` varchar(100) NOT NULL, `ans` blob NOT NULL, PRIMARY KEY (`username`) )'
 ...
 conn.connect(function (err) {
   ...
   var sql =
     'CREATE TABLE IF NOT EXISTS `discussion` ( `dsc_id` INT NOT NULL auto_increment,`dsc_namekebab` VARCHAR(500) NOT NULL, `dsc_name` VARCHAR(500) NOT NULL, `usr_id` VARCHAR(45) NULL, `thanks` INT, `data` text(60000) NULL, `post_time` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP, `total_posts` INT NOT NULL DEFAULT 0, PRIMARY KEY (`dsc_id`), UNIQUE INDEX `discussion_id_UNIQUE` (`dsc_id` ASC));'
 ...
 var sql = 'CREATE TABLE if not exists `comment_thanks` (`user_id` varchar(50) NOT NULL,`idCmt` int NOT NULL);';
 conco.query(sql, function (err, result) {
   if (err) throw err;
 });

 var sql = 'CREATE TABLE if not exists `discussion_thanks` (`user_id` varchar(50) NOT NULL,`dsc_id` int NOT NULL) ;';
 conco.query(sql, function (err, result) {
   if (err) throw err;
 });
